...
public interface ReactiveMongoOperations {
	 ...
	 */
	ComposablePublisher<Document> executeCommand(Bson command);
	 ...
	 */
	<T> ComposablePublisher<T> findAll(Class<T> entityClass);
	 ...
	 */
	ComposablePublisher<Void> insert(Object objectToSave);
	 ...
	 * @return A {@code Publisher<Void>} that signal when the data has been effectively inserted into the database.
	 */
	<T> ComposablePublisher<Void> insert(Publisher<? extends Object> objectStream, Class<T> entityClass);

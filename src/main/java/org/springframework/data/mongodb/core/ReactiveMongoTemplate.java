...
public class ReactiveMongoTemplate implements ReactiveMongoOperations {
	...
	@Override
	public ComposablePublisher<Document> executeCommand(Bson command) {
		return ComposablePublisher.from(this.database.runCommand(command));
	}

	@Override
	public <T> ComposablePublisher<T> findAll(Class<T> entityClass) {
		return ComposablePublisher.from(getCollection(entityClass).find());
	}

	@Override
	public ComposablePublisher<Void> insert(Object objectToSave) {
		return ComposablePublisher.from(getCollection((Class<Object>)objectToSave.getClass()).insertOne(objectToSave)).after();
	}

	@Override
	public <T> ComposablePublisher<Void> insert(Publisher<? extends Object> objectStream, Class<T> entityClass) {
		// TODO See https://jira.mongodb.org/browse/JAVARS-17
		return ComposablePublisher.from(objectStream).flatMap( p -> getCollection(entityClass).insertOne((T)p)).after();
	}
